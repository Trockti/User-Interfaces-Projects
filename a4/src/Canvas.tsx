...
export function Canvas({ width = 100, height = 100, shape }: CanvasProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const clicked = State.selectedIds.value.includes(shape.id);
  ...
  function draw(gc: CanvasRenderingContext2D) {
    ...
    const w = gc.canvas.width;
    const h = gc.canvas.height;
  ...
  return (
    <canvas
      class={`m-[10px] flex-none w-[50px] h-[50px]  ${type == "square"? "border-black" : "border-lightgrey"}
      ${clicked ? 'outline outline-1 outline-offset-2 outline-blue-700' : 'hover:outline hover:outline-4 hover:outline-lightblue border'}`}
      ref={canvasRef}
      width={width}
      height={height}
      onClick={handleClick}
      onMouseEnter={() => State.canClear.value = false}
      onMouseLeave={() => State.canClear.value = true}
      key={shape.id}
    />
  );
}
