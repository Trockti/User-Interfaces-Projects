...
export function CatForm() {
    ...
    if (State.selectedId.value != null && State.selected.value == 1) {
        State.shapes.value.map((shape) => {
            if (shape.id == State.selectedId.value) {
                ...
                eyeDirection = attributes.eyeDirection;
            }
        });
    }
    const [inputValue, setInputValue] = useState(hue);
    const [eyeDirectionValue, setEyeDirectionValue] = useState(eyeDirection);
    ...
    function handleEyeDirectionChange(e: Event) {
        const newValue = (e.target as HTMLSelectElement).value;
        setEyeDirectionValue(newValue);
        State.updateEyeDirection(State.selectedId.value, newValue);
    }
    ...
    return (
        <div class="flex-1 m-[10px] border border-gray-400 p-[10px] flex flex-col items-center space-y-4">
            ...
            <div class="flex flex-row justify-center">
            <label class="mr-[10px]">Look </label>

            <select class='w-[70px]'  onChange={handleEyeDirectionChange} value={eyeDirectionValue}>
            {['left', 'centre', 'right'].map((direction) => (
